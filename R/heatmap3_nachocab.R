...
heatmap.3 <- function(x,
    ...
    if (!missing(ColSideColors)) {
        ...
        if (!is.matrix(ColSideColors)){
            ...
            if (length(colnames(ColSideColors)) > 0) {
                axis(2, 0:(dim(csc)[2] - 1)/max(1,(dim(csc)[2] - 1)), colnames(ColSideColors), las = 2, tick = FALSE, cex.axis=min(1, 7*lhei[2] / ncol(csc)))
            }
        }
    }
    ...
    if (identical(colRow, NA)) {
        axis(4, iy, labels = labRow, las = 2, line = -0.5, tick = 0,
            cex.axis = cexRow)
    }
    else {
        # one call per row...
        for(i in seq(length(iy))) axis(4, iy[i], labels = labRow[i], las = 2, line = -0.5, tick = 0, cex.axis = cexRow, col.axis=rev(colRow)[i])
        # for(i in length(iy)) axis(4, iy[i], labels = labRow[i], las = 2, line = -0.5, tick = 0, cex.axis = cexRow, col.axis=rep(c('green', 'red'), length.out=length(iy))[i])
    }
    # end edit
    if (!is.null(ylab))
    ...
    # Julien rectangle overlay
    if (!missing(rect_overlay)){
        for (rects in rect_overlay) rect(xleft = rects$xleft, ybottom = rects$ybottom, xright = rects$xright, ytop = rects$ytop, lty = 1, lwd = ifelse(is.null(rects$width), 0.5, rects$width), col = "transparent", border = ifelse(is.null(rects$color), "black", rects$color))
    }
    ...
    # par(mar = c(0, 0, if (!is.null(main)) 5 else 0, margins[2])) # orig 
    par(mar = c(0, 0, if (!is.null(main)) .6 else 0, margins[2]))
    if (dendrogram %in% c("both", "column")) {
        plot(ddc, axes = FALSE, xaxs = "i", leaflab = "none")
    }
    else plot.new()
    if (!is.null(main))
        # title(main, cex.main = 1.5 * op[["cex.main"]]) # orig 
        title(main, cex.main = .8 * op[["cex.main"]]) 
    if (key) {
