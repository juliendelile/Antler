...
#' DISCLAIMER: this repository is under construction. Finalized release will be available soon.  

#' The R script used to analysis the dataset shown in *Delile, J., Rayon, T., Melchionda, M., Edwards, A., Briscoe, J., & Sagner, A. (2018). Single cell transcriptomics reveals spatial and temporal dynamics of gene expression in the developing mouse spinal cord. BioRxiv, 472415. https://doi.org/10.1101/472415* is available at https://juliendelile.github.io/MouseSpinalCordAtlas/.  

#' ![Neuronal types clustering and highlights](https://github.com/juliendelile/MouseSpinalCordAtlas/blob/master/dendrogram_highlights.png)
#'  
...
antler_path = '/camp/lab/briscoej/working/julien/Antler'
dataset_path = '/camp/lab/briscoej/working/julien/Antler/inst/extdata/10X_Mouse_181001/'

working_path = paste0(dataset_path, "/analysis_181116_cleaning/")
plot_path = working_path

partition_path = paste0(dataset_path, "/iteration14_newcolors.csv")
...
source('examples/MouseSpinalCordAtlas_tools.R')
...
m = Antler$new(plot_folder=plot_path, num_cores=15)

m$loadDataset(folderpath=dataset_path)
...
m$plotReadcountStats(data_status="Raw", by="timepoint", category="Replicate.ID", basename="preQC")

#' ![](./suppl_files/preQC_UMI_statistics_all.png)
#' ![](./suppl_files/preQC_UMI_statistics_Replicate.ID_by_timepoint.png)
#'  
...
m$plotReadcountStats(data_status="Raw", by="timepoint", category="Replicate.ID", basename="postQC")

#' ![](./suppl_files/postQC_statistics_cellNumber_Replicate.ID_by_timepoint.png)
#' ![](./suppl_files/postQC_statistics_counts_Replicate.ID_by_timepoint.png)
#' ![](./suppl_files/postQC_statistics_geneCounts_Replicate.ID_by_timepoint.png)
#'  
...
cell_partition = doCellPartition(known_template_file=partition_path, readcounts=m$getReadcounts(data_status='Raw'))

pop_colors = getPopulationColors(known_template_file=partition_path)
...
m$plotGeneModules(
                  ...
                  curr_plot_folder=plot_path
                )

#' <a href="./suppl_files/FIG1_Map_Step1_dR.genemodules_Normalized_none.pdf">Download PDF</a>

#' ![](./suppl_files/FIG1_Map_Step1_dR.genemodules_Normalized_none.png)
#'  
...
#' ![](./suppl_files/FIGS1_Map_Step2_dR.genemodules_Raw_logscaled.png)
#'  
...
write.csv(dcounts, file=paste0(plot_path, "/FIGS1_doublet_estimates.csv"))
...
write.table(markersToMask(cell_partition$step2_markers) %>% cbind("Gene"=rownames(.), .), file=paste0(plot_path,'/Table_S1.csv'), row.names=F, sep=";", quote=FALSE)
...
saveRDS(m_neural, file=paste0(working_path, '/m_neural.rds'))
# m_neural=readRDS(paste0(working_path, '/m_neural.rds'))
saveRDS(m_neuron, file=paste0(working_path, '/m_neuron.rds'))
# m_neuron=readRDS(paste0(working_path, '/m_neuron.rds'))
saveRDS(m_prog, file=paste0(working_path, '/m_prog.rds'))
# m_prog=readRDS(paste0(working_path, '/m_prog.rds'))
saveRDS(m, file=paste0(working_path, '/m_typed.rds'))
# m = readRDS(paste0(working_path, '/m_typed.rds'))
...
pdf(paste0(plot_path, '/FIG1_Progenitors_bubble_chart.pdf'), height=3.5, width=7, useDingbats=FALSE)
...
#' ![](./suppl_files/FIG1_Progenitors_bubble_chart.png)
#'  

pdf(paste0(plot_path, '/FIG1_Neurons_bubble_chart.pdf'), height=3.5, width=10, useDingbats=FALSE)
p <- bubble_chart.df %>%
...
#' ![](./suppl_files/FIG1_Neurons_bubble_chart.png)
#'  
...
print(Sys.time()-t0)
saveRDS(step1.tsne.coords, file=paste0(working_path, '/step1.tsne.coords2.rds'))

# Load pre calculated tsne coordinates
step1.tsne.coords = readRDS(paste0(working_path, '/step1.tsne.coords.rds'))

png(paste0(plot_path, '/FIG1_Map_Step1_tSNE_Normalized_Types.png'), width=4000, height=4000, pointsize=75)
...
#' ![](./suppl_files/FIG1_Map_Step1_tSNE_Normalized_Types.png)
#'  

#' Progenitor only / Neuron only

png(paste0(plot_path, '/FIG1_Map_Step1_tSNE_Normalized_Progenitor_Focus.png'), width=2000, height=2000, pointsize=40)
...
#' ![](./suppl_files/FIG1_Map_Step1_tSNE_Normalized_Progenitor_Focus.png)

png(paste0(plot_path, '/FIG1_Map_Step1_tSNE_Normalized_Neuron_Focus.png'), width=2000, height=2000, pointsize=40)
...
#' ![](./suppl_files/FIG1_Map_Step1_tSNE_Normalized_Neuron_Focus.png)
#'  
...
for(tp in unique(pData(m$expressionSet)$timepoint)) {
  ...
  rep_ids = unique(pData(m$expressionSet)$Replicate.ID[pData(m$expressionSet)$timepoint==tp])

  cell_colors = ifelse(
        pData(m$expressionSet)$timepoint != tp,
        'grey',
        rep_colors[pData(m$expressionSet)$Replicate.ID]
        )
  ...
  png(paste0(plot_path, '/FIG1SUPP_Map_Step1_tSNE_Normalized_ReplicateIDs_', tp, '.png'), width=2000, height=2000, pointsize=40)
...
#' ![](./suppl_files/FIG1SUPP_Map_Step1_tSNE_Normalized_ReplicateIDs_9.5.png)
#' ![](./suppl_files/FIG1SUPP_Map_Step1_tSNE_Normalized_ReplicateIDs_10.5.png)
#' ![](./suppl_files/FIG1SUPP_Map_Step1_tSNE_Normalized_ReplicateIDs_11.5.png)
#' ![](./suppl_files/FIG1SUPP_Map_Step1_tSNE_Normalized_ReplicateIDs_12.5.png)
#' ![](./suppl_files/FIG1SUPP_Map_Step1_tSNE_Normalized_ReplicateIDs_13.5.png)
#'  

#' Plot Sex

sex.df=cbind.data.frame(Xist=m$getReadcounts('Raw')['Xist',], pData(m$expressionSet)) %>%
      dplyr::group_by(Replicate.ID, timepoint) %>%
      dplyr::summarize(Xist_mean=mean(Xist)) %>%
      dplyr::mutate(Sex=ifelse(Xist_mean < 1, "Male", "Female")) %>%
      dplyr::arrange(timepoint)

cell_sex = pData(m$expressionSet) %>%
      dplyr::select(Replicate.ID, timepoint) %>%
      dplyr::left_join(sex.df, by=c("timepoint"="timepoint", "Replicate.ID"="Replicate.ID")) %>%
      .$Sex

sex_colors = c("Male"="cornflowerblue", "Female"="coral1")

png(paste0(plot_path, '/FIG1SUPP_Map_Step1_tSNE_Normalized_Sex.png'), width=2000, height=2000, pointsize=40)
plot(step1.tsne.coords, col=sex_colors[cell_sex], pch=16, cex=.4, main='Sex', xlab = "", ylab = "", xaxt='n', yaxt='n', bty = "n")
graphics.off()

#' ![](./suppl_files/FIG1SUPP_Map_Step1_tSNE_Normalized_Sex.png)
#'  
...
pdf(paste0(plot_path, 'FIG2_DV_Population_Ratios.pdf'), width=7, height=4, useDingbats=FALSE)
...
#' ![](./suppl_files/FIG2_DV_Population_Ratios.png)
#'  

#' Comparison with Kicheva et al., 2014

kicheva_vals = read.table(file=paste0(dataset_path, '/kicheva2014_Fig1G.csv'), sep='\t', stringsAsFactors=F, header=FALSE, skipNul=T)
...
pdf(paste0(plot_path, 'FIG2_DV_Population_Ratios_Comparison_Kicheva.pdf'), width=7, height=4, useDingbats=FALSE)
print(p)
graphics.off()

#' <a href="./suppl_files/FIG2_DV_Population_Ratios_Comparison_Kicheva.pdf">Download PDF</a>
#' ![](./suppl_files/FIG2_DV_Population_Ratios_Comparison_Kicheva.png)
#'  
...
m_neuron_combDE = resampleDataForCombinationDE(
              orig_m_neural_path = paste0(working_path, '/m_neural.rds'),
...
save(Monocle_run_neuron, file=paste0(working_path,'/Monocle_run_neuron_NegBinFamilyResample'))
# load(paste0(working_path,'/Monocle_run_neuron_NegBinFamilyResample.Rdata'))
...
saveRDS(Monocle_run_neuron_topModel, file=paste0(working_path, '/Monocle_run_neuron_topModel.rds'))
# Monocle_run_neuron_topModel = readRDS(file=paste0(working_path, '/Monocle_run_neuron_topModel.rds'))
...
saveRDS(m_neuron_combDE$dR$genemodules, file=paste0(working_path, '/neurons_combDE_genelists.rds'))
save.image(paste0(working_path, '/workspace_postCombDE_neurons.Rdata'))
...
saveRDS(gms.ordered, file=paste0(working_path, '/neurons_combDE_genelists_ordered.rds'))
...
#' ![](./suppl_files/FIG3SUPP_Neural_DE_Combinatorial_Neurons_NegBinResampled_dR.genemodules_Raw_logscaled.png)
#'  
...
save(Monocle_run_prog, file=paste0(working_path,'/Monocle_run_prog_NegBinFamilyResample.Rdata'))
# load(paste0(working_path,'/Monocle_run_prog_NegBinFamilyResample.Rdata'))

#' Filter best model, for each gene

Monocle_run_prog_topModel = getTopModel(m_prog_combDE, Monocle_run_prog, temporal_pattern_thres=.2)

saveRDS(Monocle_run_prog_topModel, file=paste0(working_path, '/Monocle_run_prog_topModel.rds'))
# Monocle_run_prog_topModel = readRDS(file=paste0(working_path, '/Monocle_run_prog_topModel.rds'))
...
save.image(paste0(working_path, '/workspace_postCombDE_progenitors.rds'))
saveRDS(m_prog_combDE$dR$genemodules, file=paste0(working_path, '/progenitors_combDE_genelists.Rdata'))
...
plot_basename_prog = paste0('FIG3SUPP_Neural_DE_Combinatorial_Progenitors_NegBinResampled')
...
#' ![](./suppl_files/FIG3SUPP_Neural_DE_Combinatorial_Progenitors_NegBinResampled_dR.genemodules_Raw_logscaled.png)
#'  
...
neuron_markers_selection = names(gene_categories) %>% {setNames(lapply(., function(gc){intersect(unname(unlist(readRDS(paste0(working_path, '/neurons_combDE_genelists_ordered.rds')))), gene_categories[[gc]])}), .)}

# remove gene whose topmodel is in dl6 (population is very small)
dl6_only_genes = Monocle_run_neuron_topModel %>% dplyr::filter(model=="m000001000000" & genename %in% unname(unlist(readRDS(paste0(working_path, '/neurons_combDE_genelists_ordered.rds'))))) %>% .$genename
...
write(jsonlite::toJSON(neuron_markers_selection, pretty=TRUE), paste0(plot_path, '/FIG3_Prediction_NeuronHighlights.json'))
...
plotPredictedSelection(currm=m_neuron,
                    ...
                    selected_genes=unname(unlist(readRDS(paste0(working_path, '/neurons_combDE_genelists.rds')))),
...
#' ![](./suppl_files/FIG3_Neuron_Marker_Prediction.png)
#'  
...
#' ![](./suppl_files/FIG3_Gene_expression_dynamics_Cldn3.png)
#'  
...
denovo_folder = paste0(plot_path, "/subClustering_Neuron_by_TypeStep2/")
...
saveRDS(m_pops, file=paste0(working_path, '/All_Clusters_topCorrDR_Neurons.rds'))

#' Partition neurons from curated gene modules  

subclustering_params = rjson::fromJSON(file=paste0(dataset_path, '/subclustering_typestep2_181106.json'))
...
saveRDS(m_pops2, file=paste0(working_path, '/All_Clusters_topCorrDR_Neurons_v2.rds'))
# m_pops2 = readRDS(file=paste0(working_path, '/All_Clusters_topCorrDR_Neurons_v2.rds'))
...
write.table(curated_gms.df, file=paste0(plot_path,'/Table_S2.csv'), row.names=F, sep=";", quote=FALSE)
...
write.table(domain_stats, file=paste0(plot_path,'/Table_S4.csv'), row.names=F, sep=";", quote=FALSE)
...
pdf(paste0(plot_path, '/FIG4_subclusters_dendrogram_full.pdf'), height=3, pointsize=2, useDingbats=FALSE)
heatmap.3(
      data_time,
      Colv=dend_merge_all,
      Rowv=FALSE,
...
#' ![](./suppl_files/FIG4_subclusters_dendrogram_full.png)
#'  

#' Aggregate all domains heatmaps in a single pdf
agg_subclust_name = paste0(plot_path, "/FIG4_Domain_sub_clustering.pdf")
...
pdf(paste0(plot_path, '/FIG5_subclustering_map_', mgnames,'.pdf'), height=3, width=10, useDingbats=FALSE)
print(p)
graphics.off()

#' <a href="./suppl_files/FIG5_subclustering_map_custom.pdf">Download PDF</a>
#' ![](./suppl_files/FIG5_subclustering_map_custom.png)
#'  
...
pdf(paste0(plot_path, '/FIG5_subclustering_map_', mgnames,'.pdf'), height=7, width=12, useDingbats=FALSE)
print(p)
graphics.off()

#' <a href="./suppl_files/FIG5_subclustering_map_all.pdf">Download PDF</a>
#' ![](./suppl_files/FIG5_subclustering_map_all.png)
#'  
...
saveRDS(m_neural, file=paste0(working_path, '/m_neural_subtypes.rds'))
# m_neural = readRDS(file=paste0(working_path, '/m_neural_subtypes.rds'))
...
pdf(paste0(plot_path, "FIG5_Domain_dynamics_ventral.pdf"), width = 10, height =2.5, useDingbats = FALSE)
print(gg)
graphics.off()

#' <a href="./suppl_files/FIG5_Domain_dynamics_ventral.pdf">Download PDF</a>
#' ![](./suppl_files/FIG5_Domain_dynamics_ventral.png)
#'  
...
#' ![](./suppl_files/GM_RP_byDV_Resampled_Selected_dR.genemodules_Raw_logscaled.png)
#'  
...
pdf(paste0(plot_path, '/gene_DV_pval.pdf'), useDingbats=FALSE)
...
saveRDS(selected_gms, paste0(working_path, '/DV_debiased_GMs.rds'))
# selected_gms = readRDS(paste0(working_path, '/DV_debiased_GMs.rds'))
...
#' ![](./suppl_files/Fig6A_PCA_HEXplot.png)
#'  
...
pt_genes.df = read.table(file=paste0(dataset_path, '/pseudotime_genes_short.csv'), sep='\t', stringsAsFactors=F, header=TRUE, skipNul=T) %>% 
...
plotNeurogenesisHeatmap(
                ...
                plotfullname=paste0(plot_path, '/FIG6C_Neurogenesis_heatmap.pdf'),
                num_pt=num_pt,
                plotted_DV=2:12,
                zscore_cap = NULL,
...
#' ![](./suppl_files/FIG6C_Neurogenesis_heatmap.png)
#'  
...
for(n in names(highlighted_genes)){
  ...
  pdf(paste0(plot_path, '/PT_profile_', n, '.pdf'), height=4, useDingbats=FALSE)
  print(p)
  graphics.off()

}

#' ![](./suppl_files/PT_profile_p3-V3.png)
#' ![](./suppl_files/PT_profile_p0-V0.png)
#' ![](./suppl_files/PT_profile_dp3-dl3.png)
#' ![](./suppl_files/PT_profile_dp2-dl2.png)
#'  

#' Aggregate all domains heatmaps in a single pdf
system2(command="gs", args=sprintf("-dBATCH -dNOPAUSE -q -sDEVICE=pdfwrite -dAutoRotatePages=/None -sOutputFile=%s %s", paste0(plot_path, "/FIG6_S6_Pseudotime_Selected_Profiles_per_Domain_Monocle.pdf"), paste0(paste0(plot_path, '/PT_profile_', names(highlighted_genes), '_Monocle.pdf'), collapse=" ")))
